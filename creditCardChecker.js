...
function validateCred(array) {
  ...
  arr_index = array.length
  for (let i = arr_index-2; i>=0;i--){
    //console.log(i)
    if ((i)%2 === 0){
      ans = array[i] * 2;
      if (ans > 9){
    ...
    newArray.unshift(ans);
  }
  newArraysum = newArray.reduce((accumulator, currentValue) => {
...
function findInvalidCards(nestedarray) {
  ...
  for (array of nestedarray){
    check = validateCred(array);
    if (check == false){
      newInvalidArray.push(array);
    }
  }
  return newInvalidArray;
  //console.log(newInvalidArray);
};
...
function idInvalidCardCompanies(nestedarray){
 let invalidCardsCompany = []
  for (array of nestedarray){
    ...
    switch(firstelement){
      case 3:
        if (invalidCardsCompany.indexOf('Amex') === -1){
          invalidCardsCompany.push('Amex');
        }
      break;

      case 4:
        if (invalidCardsCompany.indexOf('Visa') === -1){
          invalidCardsCompany.push('Visa');
        }
      break;

      case 5:
        if (invalidCardsCompany.indexOf('Mastercard') === -1){
          invalidCardsCompany.push('Mastercard');
        }
      break;

      if (invalidCardsCompany.indexOf('Discover') === -1){
          invalidCardsCompany.push('Discover');
        }
      break;
